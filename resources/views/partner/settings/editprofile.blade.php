...
@endphp
    ...
    <div class="lg:flex hidden lg:bg-white lg:mb-[0px] lg:px-[17px] lg:py-[15px] mb-[20px] justify-start items-center">
        <div class="flex justify-start items-center w-1/2">
            <a href="{{route('partner.profile')}}" class="links_item_cta inline-block p-2 mr-2 border border-[#C8C0C0] rounded-md hover:border-[#F3CD0E] active:border-[#000] active:bg-[#F3CD0E]">
        ...
        <div class="flex justify-end items-center w-1/2">
            <a href="{{route('partner.profile')}}" class="links_item_cta dash_circle desh_view_bg font-medium text-gray-600 hover:underline">
...
<div class="pt-[42px] px-[36px] lg:px-[17px] lg:py-[25px] xl:py-10 bg-[#F6F6F6] lg:mb-[60px] lg:flex lg:justify-center pb-[100px]  min-h-screen">
    ...
    <div class="max-w-[768px] w-full bg-white lg:bg-[#F6F6F6] rounded-[10px] px-12 lg:px-[0px] py-9 lg:py-[0px]">
        <div class="booking_section">
            <form action="{{route('partner.profile.update',$userId)}}" method="post" onsubmit="return validate();">
            @csrf
     ...
     <div class="pb-5">
               <div class="flex flex-wrap -mx-3">
                        <div class="w-1/2 px-3 sm:w-full sm:pb-5">
                            <label for="primary_manager"class="block pb-2.5 font-normal leading-4 text-left text-black text-sm">Primary Manager</label>
                            <input id="primary_manager" tabindex="-1" class="not_required_fields w-full px-5 py-3 font-normal leading-normal text-black bg-white border rounded-md border-black500 focus:outline-none focus:border-siteYellow alpha_validate text-base "
                            placeholder="Enter primary manager" type="text" name="primary_manager"value="{{Helper::getUserMeta($userId,'primary_manager')}}">
                            <span class="required text-sm"></span>
                        </div>
                        <div class="w-1/2 px-3 sm:w-full not_require_container">
                            <label for="primary_manager_mobile" class="block pb-2.5 font-normal leading-4 text-left text-black text-sm">Primary Manager Mobile</label>
                            <input id="primary_manager_mobile" tabindex="-1" placeholder="Enter primary manager mobile"
                                class="not_required_fields primary_manager_mobile phone-input phone_validation_manager w-full px-5 py-3 font-normal leading-normal text-black bg-white border rounded-md border-black500 focus:outline-none focus:border-siteYellow text-base "  type="text" name="primary_manager_mobile"   data-dial-code="{{Helper::getUserMeta($userId, 'primary_manager_country_code')}}"  value="{{Helper::getUserMeta($userId, 'primary_manager_country_code')}} {{Helper::getUserMeta($userId, 'primary_manager_mobile')}}"
                                minlength="10">
                                <input type="hidden" name="primary_manager_country_code" class="primary_manager_country_code" value="{{Helper::getUserMeta($userId, 'primary_manager_country_code')}}">
                                <span class="required text-sm"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="pb-5">
                    <div class="flex flex-wrap -mx-3">
                        ...
                        <div class="w-1/2 px-3 sm:w-full sm:pb-5">
                            <label for="secondary_manager"class="block pb-2.5 font-normal leading-4 text-left text-black text-sm">Secondary Manager</label>
                            <input id="secondary_manager" tabindex="-1" class="not_required_fields w-full px-5 py-3 font-normal leading-normal text-black bg-white border rounded-md border-black500 focus:outline-none focus:border-siteYellow alpha_validate text-base " placeholder="Enter secondary manager" type="text" name="secondary_manager"
                            value="{{Helper::getUserMeta($userId,'secondary_manager')}}">
                            <span class="required text-sm"></span>

                        </div>

                        <div class="w-1/2 px-3 sm:w-full not_require_container">
                            <label for="secondary_manager_mobile" class="block pb-2.5 font-normal leading-4 text-left text-black text-sm">Secondary Manager Mobile</label>
                            <input id="secondary_manager_mobile" tabindex="-1" placeholder="Enter secondary manager mobile"
                                class="not_required_fields secondary_manager_mobile  phone_validation_manager w-full px-5 py-3 font-normal leading-normal text-black bg-white border rounded-md border-black500 focus:outline-none focus:border-siteYellow text-base "
                                type="text" name="secondary_manager_mobile"
                                data-dial-code="{{ Helper::getUserMeta($userId, 'secondary_manager_country_code') }}"
                                value="{{Helper::getUserMeta($userId, 'secondary_manager_country_code')}} {{ Helper::getUserMeta($userId, 'secondary_manager_mobile') }}">
                            <input type="hidden" name="secondary_manager_country_code" class="secondary_manager_country_code" value="{{Helper::getUserMeta($userId, 'secondary_manager_country_code')}}">
                            <span class="required text-sm"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="pb-5">
                    <div class="flex flex-wrap -mx-3">
                        <div class="w-full px-3 sm:w-full inp_container">
                            <label for="partner_short_name"
                                class="block pb-2.5 font-normal leading-4 text-left text-black text-sm">
                                Short Name<span class="text-[#ff0000]">*</span>
                            </label>
                            <input id="partner_short_name" tabindex="-1"
                                ...
                                type="text" name="partner_short_name" placeholder="Enter partner short name"
                                data-error-msg="Partner short name is required" value="{{ strtoupper(Helper::getUserMeta($userId,'partner_short_name'))}}">
                            <span class="required text-sm"></span>
                            @if ($errors->has('partner_short_name'))
                            <div class="error text-sm">
                                <ul>
                                    <li>{{ $errors->first('partner_short_name') }}</li>
                                </ul>
                            </div>
                            @endif
                        </div>
...
function validate() {
    ...
    var primary_manager = $('#primary_manager').val().trim();
    var primary_manager_mobile = $('#primary_manager_mobile').val().trim();

    if (primary_manager === '' && primary_manager_mobile !== '') {
        console.log('Error: Primary Manager is required');
        $('#primary_manager').siblings('.required').html('Primary Manager is required');
        hasErrors = true;
    } else if (primary_manager !== '' && primary_manager_mobile === '') {
         console.log('Error: Primary Manager Mobile is required');
        var primaryManagerMobileElement = $('#primary_manager_mobile');
         $('#primary_manager_mobile').closest('.not_require_container').find('.required').html('Primary Manager Mobile is required');
         if (primary_manager_mobile.length < 10) {
            if (!firstErrorElement) {
            firstErrorElement = $('#primary_manager_mobile');
            }
             numErrors = true;
        }
         hasErrors = true;
    ...
    var secondary_manager = $('#secondary_manager').val().trim();
    var secondary_manager_mobile = $('#secondary_manager_mobile').val().trim();

    if (secondary_manager === '' && secondary_manager_mobile !== '') {
        console.log('Error: Secondary Manager is required');
        $('#secondary_manager').siblings('.required').html('Secondary Manager is required');
        hasErrors = true;

    } else if (secondary_manager !== '' && secondary_manager_mobile === '') {
        console.log('Error: Secondary Manager Mobile is required');
        var primaryManagerMobileElement = $('#secondary_manager_mobile');
        $('#secondary_manager_mobile').parents('.not_require_container').find('.required').html('Secondary Manager Mobile is required');
        if (secondary_manager_mobile.length < 10) {
            if (!firstErrorElement) {
            firstErrorElement = primaryManagerMobileElement;
            }
            numErrors = true;
        }
        hasErrors = true;
        ...
        if (secondary_manager_mobile.length < 10) {
            if (!firstErrorElement) {
            firstErrorElement = $('#secondary_manager_mobile');
            }
            $('#secondary_manager_mobile').parents('.not_require_container').find('.required').html('Secondary Manager Mobile is required');
            numErrors = true;
        }
   ...
   $('body').on('input','.phone_validation,.phone_validation_manager',function(){
   ...
   $('#partner_short_name').on('input', function(){
    ...
    $('body').on('keyup','.phone_validation_manager',function(){
    var inputValue = $(this).val().trim();
    var validationSpan = $(this).closest('.not_require_container').find('.required');
    if (inputValue.length < 10) {
        validationSpan.html("Mobile number must be at least 10 digits");
    }
    else{
    ...
    var fileTypesBusLicense = ['jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG', 'webp', 'WEBP'];
        ...
        function upload(img, extension) {
            ...
            $(".overlay_sections").css("display", "block");
            jQuery.ajax({
                url: "{{ route('partner.profile.photo.update',1) }}",
                data: form_data,
                type: 'POST',
                contentType: false,
                processData: false,
...
$('body').on('input', '.phone_validation, .phone_validation_manager', function() {
   ...
   $(this).val(numericValue);
    ...
    const input2 = document.querySelector("#primary_manager_mobile");
    ...
    var intlNumber = $(input2).siblings('.iti__flag-container').children('.iti__selected-flag').siblings('.iti__country-list').children('.iti__active').find('.iti__dial-code').text();
    console.log('primary_manager_country_code',intlNumber);
    $('.primary_manager_country_code').val(intlNumber);
    });

    const input3 = document.querySelector("#secondary_manager_mobile");
    var input3CountryCode = '';
    ...
    input3.addEventListener("countrychange", function() {
        var secondary_manager_country_code = $(this).closest('.secondary_manager_mobile').find('.iti__selected-dial-code').html();
        $('.secondary_manager_country_code').val(secondary_manager_country_code);
        console.log('secondary_manager_country_code',secondary_manager_country_code);
    });
    ...
    if ($('#primary_manager_mobile').val().trim() === '' || $('#primary_manager_mobile').val().trim() === '0') {
        $('#primary_manager_mobile').val('').click();
    }
    ...
    if ($('#secondary_manager_mobile').val().trim() === '' || $('#secondary_manager_mobile').val().trim() === '0') {
        $('#secondary_manager_mobile').val('').click();
    }
