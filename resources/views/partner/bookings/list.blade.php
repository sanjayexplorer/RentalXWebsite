...
<style>
    ...
    .checkmark::after { content: ""; position: absolute; display: none; left: 6px; top: 2px; width: 5px; height: 10px; border: solid #fff; border-width: 0 2px 2px 0; -webkit-transform: rotate(45deg); -ms-transform: rotate(45deg); transform: rotate(45deg); }
    input.choice_checkBox { position: absolute; opacity: 0; cursor: pointer; width: 0; height: 0; }
                    ...
                    <div class="relative">
                        ...
                        <div class="bg-[#FFFFFF]">
                            <div class="w-full overflow-y-auto filter_height_sec">
                                <form action="{{route('partner.booking.list.filter')}}" id="mob_filters_form" method="post"
                                    ...
                                    <div class="list_cr_out flex items-center  @if(strcmp($booked_car->customer_name,'')!==0 && strcmp($booked_car->customer_mobile_country_code,'')!==0 && strcmp($booked_car->customer_mobile,'')!==0) justify-center @else justify-start @endif">
                                        <div class="list_cr_item">
                                            <div class="booking_list_card_top_section "data-id="{{ $booked_car->id }}">
                                               ...
                                               <div class="flex">
                                                <div class="">
                                                <div class="flex items-center flex-wrap gap-[3px]" data-bookingTopSec="{{$booked_car->booking_owner_id}}">
                                            ...
                                            <div class="flex flex-wrap  booking_list_card_bottom_section mt-[9px]">
                                                ...
                                                <div class="ctc_info_w">
                                                    <div class="mb-[4px]">
                                                        ...
                                                        <h4 class="capitalize text-[#2B2B2B] font-medium leading-4 text-[13px]">
                                                            @if(strcmp($booked_car->carId,'0')!==0)
                                                            {{Helper::getCarNameByCarId($booked_car->carId)}}
                                                            @else
                                                            {{$booked_car->car_name}}
                                                            @endif
                                                            </h4>
                                                        <p class="uppercase text-[#666] text-[13px] font-normal">
                                                            @if(strcmp($booked_car->carId,'0')!==0)
                                                            {{Helper::getCarRagisterationNoByCarId($booked_car->carId)}}
                                                            @else
                                                            {{$booked_car->registration_number}}
                                                            @endif
                                                        </p>
                                                    </div>
                                                    <div class="block">
...
// Function to load more cars
    ...
    function loadMoreCars(a) {
        ...
        if(search_list){
            ...
            if (match) {
                ...
                var countryCode = "+" + match[2]; // Concatenate '+' sign
        ...
        $.ajax({
            url: '{{ route('partner.loadMoreBookings') }}',
            type: 'GET',
            data: {
                offset: offset,
    ...
    $('#mob_fltr_apply_btn').on('click',function(e){
        ...
        if ((mobile_fltr_start_date !== '') || (mobile_fltr_end_date !== ''))
        {
            ...
            $.ajax({
            url:"{{route('partner.booking.list.filter')}}",
            type: 'POST',
            data:{
                    '_token':'{{ csrf_token() }}',
    ...
    $('.choice_checkBox').on('click', function() {
        ...
        $.ajax({
                url: "{{ route('partner.searchCustomerAndMob') }}",
                type: 'GET',
                dataType: 'json',
                data: {
                    '_token': '{{ csrf_token() }}',
...
/////////////////////////////////////////  DESKTOP  /////////////////////////////////////////
    ...
    $('#desktop_fltr_apply_btn').on('click',function(e)
    {
        ...
        if ( (desktop_start_date !== '') || (desktop_end_date !== '') )
        {
            ...
            // console.log("countryCodee", countryCode); // Logs '+91'
    ...
    $("#search_listing").autocomplete({
        source: function (request, response) {
            ...
            $.ajax({
                url: '{{ route('partner.autocompleteCustomerAndMob') }}',
                type: 'GET',
                dataType: "json",
                data: {
                    search: request.term
    ...
    $('body').on('click', '.booking_action_btn', function(e) {
        ...
        if(result.isConfirmed){
            ...
            $.ajax({
                url:"{{ route('partner.change.booking.status')}}",
                type: 'POST',
                data: {
                    '_token': '{{ csrf_token() }}',
                    'action': action,
...
$('body').on('click', '.search_booking_action_btn', function(e) {
    ...
    if(result.isConfirmed){
        ...
        $.ajax({
            url:"{{ route('partner.search.change.booking.status')}}",
            type: 'POST',
            data: {
                '_token': '{{ csrf_token() }}',
                'action': action,
