...
#users_list_paginate a{ color: #BEBEBE;}
...
<div>
    <div class="right_dashboard_section_inner py-9 px-9 lg:py-[20px] lg:px-[17px] bg-[#F6F6F6]  min-h-screen">
                ...
                <div class="relative ta_scrollbar_out bg-[#fff] md:pb-[30px] pb-[55px] md:pb-[30px] md:mb-[60px] overflow-x-auto overflow-y-hidden rounded-[10px] shadow-md sm:rounded-lg">
                    ...
                    <th scope="col" class="pl-3 pr-[90px]  pt-[20px] pb-3 md:pt-[14px] text-right text-[#000] font-bold">
                            Action
                        </th>
                    </tr>
                    </thead>
...
<script>
   ...
   var dataTable = $('#users_list').DataTable({
    ...
    order: [[0, "desc"]],
    ajax: {
    url: "{{route('partner.users.list.ajax')}}",
    },
    columns: [
    ...
    createdRow: function (row, data, dataIndex) {
        ...
        $('td:eq(0)',row).html(
            '<div class=""> <div class=" sm:text-sm sm:text-[#000000] sm:font-normal">'+ data.name +'</div> <div class="hidden md:block sm:text-xs text-[#444444]"> <span class="sm:text-xs md:text-sm sm:block">'+ data.contact +'</span> </div> </div>'
        );
        ...
        $(row).find('td:eq(3)').addClass('pl-3 pr-5 py-4  text-right md:py-2.5 md:pr-3 ');
        $('td:eq(3)', row).html(
            ...
            '</a>' +
            '<div class=" relative  triple_dots_main flex py-2 rounded-t border border-transparent "> <a href="javascript:void(0);" class=" relative  triple_dots_action_btn triple_dots ">' +
            '<img src="{{asset('images/triple_dottts.svg')}}" alt="More Options">' +
            '</a>' +
            ' <div class="dots_hover_sec list_link_frame_section top-[42px] absolute w-[165px] border border-[#D1D1D1] rounded-[5px] rounded-tr-none py-[10px] bg-[#fff] z-[2] -left-[129px] custom-shadow afclr hidden"> <ul class="frame_list_items_popup">' +
            ...
            cancelButtonText: 'NO, CANCEL'
                ...
                if (result.isConfirmed) {
                    ...
                    url: deleteUrl,
                        ...
                        if (data.success) {
                        //    console.log('under success part',data,'click:',$(that).parents('tr'));
                        ...
                        Swal.fire({
                            ...
                            if(result.isConfirmed){
                                checkTableLength();
                                if((tableRowLen-1)==0){
                                    console.log('under result is confirmed true:',tableRowLen);
                                ...
                                checkTableLength();
                                  ...
                                  if((tableRowLen-1)==0){
                                    console.log('under result is confirmed false:',tableRowLen);
                                    $('#users_list').DataTable().ajax.reload();
                                }
                            }
                        });
