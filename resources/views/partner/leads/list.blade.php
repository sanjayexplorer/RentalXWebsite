    ...
    <div class="right_dashboard_section_inner py-9 px-9 lg:py-[20px] lg:px-[17px] bg-[#F6F6F6] min-h-screen">
            ...
            <div class="flex items-center mb-[36px] lg:mb-[20px]">
                ...
                <div class="w-1/2 text-right flex items-center justify-end ">
                    <a href="{{route('partner.leads.add')}}" class="links_item_cta inline-flex rounded-[4px] items-center text-black text-base md:text-sm font-normal bg-siteYellow px-[20px] py-2.5 md:px-[15px] ">
...
<script>
    ...
    var dataTable = $('#leads_list').DataTable({
        ...
        order: [[0, "desc"]],
        ajax: {
        url: "{{route('partner.leads.list.ajax')}}",
        },
        columns: [
                    ...
                    url: deleteUrl,
                        ...
                        Swal.fire({
                            ...
                            if(result.isConfirmed){
                                ...
                                checkTableLength();
                                //  let tableRowLen=  $('leads_list').find("tr").length
                            ...
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    if(result.isConfirmed){
                                        $.fancybox.close();
                                        $('#leads_list').DataTable().ajax.reload();
                                        console.warn('under success:',data.status);
