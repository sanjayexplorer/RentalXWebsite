...
class BookingController extends Controller
{
    ...
    public function list()
    {
        ...
        foreach ($allBookedCars as $value) {
            ...
            if($pickup_date_time_after_oneday->lt($currentDateTime) && $dropoff_date_time_after_oneday->lt($currentDateTime)){
                ...
                // this will check if the current date/time  is one day later  than pickup date/time
                ...
                // this will check if the current date/time  is one day later  than dropoff date/time
    ...
    public function loadMoreBookings(Request $request)
    {
        ...
        if(count($booked_cars))
        {
            ...
            return response()->json(['success' => true, 'error' => false, 'booked_cars'=> $booked_cars,'desktopBookingHtml'=>$desktopBookingHtml, 'mobileBookingHtml'=> $mobileBookingHtml, 'test'=> $test, 'offset'=> $offset]);
    ...
    public function calendar(Request $request)
    {
        ...
        if ($requestData) {
            ...
            $cars = cars::select(
                ...
                if (!empty ($formArray['car_name']) && (empty ($formArray['car_type']) && empty ($formArray['transmission'])) ) {
                    // echo 'only car_name';
                    $query->whereIn('cars.name', $formArray['car_name']);
                    dd($query);
                }

                if (!empty ($formArray['car_type'])  && (empty ($formArray['car_name']) && empty ($formArray['transmission']))) {
                    // echo 'only car_type';
                    $query->whereIn('cars.car_type', $formArray['car_type']);
                    // dd($query);
                }

                if (!empty ($formArray['transmission'])  && (empty ($formArray['car_type']) && empty ($formArray['car_name']))) {
                    // echo 'only transmission';
                    $query->whereIn('cars.transmission', $formArray['transmission']);
                    // dd($query);
                }

                // check with car_type with transmission start
                if (!empty ($formArray['transmission']) && !empty ($formArray['car_type']) && (empty ($formArray['car_name']))) {
                ...
                if (!empty ($formArray['car_type']) && !empty ($formArray['transmission']) && !empty ($formArray['car_name'])) {
                    // echo 'when all filters are not empty';
                    $query->whereIn('cars.car_type', $formArray['car_type'])->whereIn('cars.transmission', $formArray['transmission'])->whereIn('cars.name', $formArray['car_name']);
                    // dd($query);
                }
            ...
            $cars = cars::where('status', '!=', 'deleted')->where('user_id', $userId)->orderByDesc('created_at')->take(10)->get();
        }
        ...
        $filtersCount= $this->checkFiltersCount($start_date,$end_date,$car_type,$car_name,$transmission);

     return view('partner.bookings.calendar', compact('dates','cars','allCarsFilter','startDate','endDate','start_date','end_date','car_name','car_type','transmission','filtersCount'));
    }
    ...
    public function calendarPost(Request $request)
    {
        ...
        return response()->json(['success' => false, 'error' => true, 'msg' => 'Please select a valid date, Either dates are Locked or Booked !!' ]);
            ...
            // dd('pickup_location:'.$pickup_location.'      dropoff_location:'.$dropoff_location);
    ...
    public function loadMoreList(Request $request)
    {
      ...
      if( !empty($fltrStartDate) ||  !empty($fltrEndDate) ||  !empty($carType) || !empty($carName) ){
           ...
           $cars = cars::where('status', 'NOT LIKE', 'deleted')
            ...
            ->where(function ($query) use ($fltrStartDate, $fltrEndDate, $carName, $carType) {
                if (!empty($carName)) {
                ...
                if (!empty($carName) && !empty($carType)) {
                ...
                <div class="car_status_row_sec pl-[15px] right_section md:flex md:flex-row  flex  md:items-start">
                    <div class="car_details_content md:flex-col md:items-start flex justify-start items-center">
                        ...
                        <div class="car_name_sec">
                            <a href="'.route('partner.car.view',$car->id).'" class=" links_item_cta text-base md:text-center font-medium capitalize showcar_title_b hover:underline transition-all duration-300 ease-out   text-purple md:font-bold md:text-sm">
    ...
    public function add(Request $request)
    {
        ...
        if ($car && $bookingStatus) {
            ...
            return view('partner.bookings.add', compact('car', 'carId','userId','firstDate', 'lastDate', 'pickupTime', 'dropoffTime', 'bookingStatusId','perDayRentalCharges','totalDayRentalCharges','diffInDays','drivers','action_type'));
        } else {
    ...
    public function addPost(Request $request, $carId)
    {
        ...
        $trimAdvanceAmount = 0;
        $registeration_number = ($request->registration_number)?$request->registration_number:'';
        ...
        if (strcmp($request->booking_type, 'external') == 0) {
           ...
           $flag = $this->checkBooking($carId, $pickupDate, $dropoffDate,$actionType);
        }
        ...
        if($flag==false)
        {
            ...
            $booking = CarBooking::create([
             ...
             $maxBookingId = CarBooking::where('user_id', $user->id)->max('bookingId');
            ...
            // $car= cars::whereId($booking->carId)->first();

            // $data[] = '';
            // $usersarray['customer_name']=$booking->customer_name;
            // $data['customer_name']=$booking->customer_name;
            //  $company_name= Helper::getUserMeta(auth()->user()->id ,'company_name');
            //  $usersarray['company_name']=$company_name;
            //  $data['company_name']=$company_name;
            //  $usersarray['bookingId']=$booking->bookingId;
            //  $data['bookingId']=$booking->bookingId;
            //   $usersarray['customer_mobile_country_code']=$booking->customer_mobile_country_code;
            //  $usersarray['customer_mobile']=$booking->customer_mobile;
            //  $data['customer_mobile']=$booking->customer_mobile;
            //  $data['customer_mobile_country_code']=$booking->customer_mobile_country_code;
            ...
            //  $data['dropoff_time']=$booking->dropoff_time;
            //  $data['start_date']= date('Y-m-d', strtotime($booking->start_date));
            //  $data['end_date']=date('Y-m-d', strtotime($booking->end_date));
            //  $data['car']=$car->name;
            //  $data['registration_number']=$car->registration_number;
            //  $data['rental_cost']=$booking->due_at_delivery;


             // sent mail to customer
            //   try {
            //             Mail::send('mails/partner/bookings/customer', $data, function($message) use ($usersarray) {

            //             $message->to('ajay.bisht@bytegrow.com',$usersarray['customer_name'])->subject

            //             ("Booking Confirmation: #" .$usersarray['bookingId'].'-'.$usersarray['company_name']);

            //             $message->from(env('MAIL_FROM_ADDRESS'),env('FROM_NAME'));
            ...
            return redirect()->route('partner.booking.calendar')->with('warning', 'Please select valid date, Either dates are Locked or Booked !!');
        }

    }

    /////// this will give filters count  //////
    private function checkFiltersCount($start_date,$end_date,$car_type,$car_name,$transmission)
    {
        $count=0;
    ...
    public function ajaxDatesFilter(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            })->orderBy('cars.created_at', 'DESC')->take($cars_length)->get();
            ...
            $cars = cars::where('status', '!=', 'deleted')->whereIn('id',$carIds)->where('user_id','=',$userId)->orderByDesc('created_at')->take($cars_length)->get();
            $date_data = '';
            $main_loop = '';
    ...
    public function view($id)
    {
        if (!$id) {
            ...
            return view('partner.bookings.view', compact('drivers','car_booking', 'car', 'carId', 'booking_payments', 'partner','assignedDriver'));
        }
    }
    ...
    public function edit($id)
    {
        if (!$id) {
            ...
            $drivers= Drivers::where('userId','=',$userId)->get();
            return view('partner.bookings.edit', compact('car_booking','car','carId','drivers'));
        }
    }
                            ...
                            $desktopBookingHtml .= '
                                        ...
                                        <div class="image_l_wh">
                                                ...
                                                <div class="ctc_info_w">
                                                    ...
                                                    <div class="block">
                                                        <a href="' . route('partner.booking.view', $booked_car->id) . '" class="links_item_cta inline-block text-[#2B2B2B] font-medium pb-[1px] leading-4 text-[13px] border-b-2 border-siteYellow">View Booking
                                                            </a>
                                                    </div>
                                                </div>
                                    ...
                                    if(strcmp($booked_car->customer_name,'')!==0 || strcmp($booked_car->customer_mobile_country_code,'')!==0 || strcmp($booked_car->customer_mobile,'')!==0){
                                      $desktopBookingHtml .= ' <div class="list_cr_item">
                                            <div class="booking_list_card_top_section">
                                                <div class="block">
            ...
            <div class="bookingLength rounded-[8px] p-[14px] md:block hidden mb-[20px]';
                    ...
                    <div class="mt-[18px] sm:mt-[12px] afclr">
                        ...
                        <div class="block mt-[2px]">
                            <a href="' . route('partner.booking.view', $booked_car->id) . '" class=" links_item_cta inline-flex items-center text-[#2B2B2B] font-medium leading-4 text-[14px]">View Booking <img src="' . asset('images/arrow-booking.svg') . '" alt="arro" class="ml-[9px] w-[24px]"></a>
                        </div>
                    </div>';
...
//////////////////////////////////////////// ALL BOOKINGS /////////////////////////////////////////////////
    ...
    public function allStatusChange(Request $request)
    {
        ...
        if(strcmp($request->checkStatus, 'list') === 0)
        {
            $booked_cars = getAllBookedCarsQuery()
            ->take($bookingLength)
            ->get();
        }
        elseif((strcmp($request->checkStatus, 'desktop_search') === 0) || (strcmp($request->checkStatus, 'mobile_search') === 0))
    ...
    public function editPost(Request $request ,$id)
    {
        ...
        //  $booking= CarBooking::whereId($id)->first();
        ...
        //   $data[] = '';
            ...
            //         {
                    ...
                    //     try {
                    //     Mail::send('mails/partner/bookings/company', $data, function($message) use ($usersarray) {

                    //     $message->to('sanjay.bareth@bytegrow.com',$usersarray['customer_name'])->subject

                    //     ("New Booking Alert: #" .$usersarray['bookingId'].'-'.$usersarray['customer_name']);
    ...
    public function ExternalBooking()
    {
        ...
        return view('partner.bookings.external',compact('drivers'));
    }
