...
class ProfileController extends Controller{
    ...
    public function profile(){
       ...
       return view('partner.settings.viewprofile',compact('userId','user'));
    }
    public function EditProfile(){
       $userId = Auth::user()->id;
       $user = User::where('status', 'NOT LIKE', 'inactive')->where('id','=',$userId)->first();

       return view('partner.settings.editprofile',compact('userId','user'));
    }
    ...
    public function ProfileUpdate(Request $request ,$id){
        ...
        $messages = [
            ...
            'partner_short_name.required' => 'Partner short name is required',
       ...
       if ($validator->fails()) {
        ...
        $this->insertOrUpdateUsermeta($id,'primary_manager', $request->primary_manager);
        $this->insertOrUpdateUsermeta($id,'secondary_manager', $request->secondary_manager);
        $this->insertOrUpdateUsermeta($id,'street_name', $request->street_name);
        $this->insertOrUpdateUsermeta($id,'primary_manager_mobile', $request->primary_manager_mobile);
        $this->insertOrUpdateUsermeta($id,'secondary_manager_mobile', $request->secondary_manager_mobile);
        ...
        if (strcmp($request->primary_manager_mobile, "") != 0) {
            $this->insertOrUpdateUsermeta($id,'primary_manager_country_code', $request->primary_manager_country_code);
        }
        if (strcmp($request->secondary_manager_mobile, "") != 0) {
            $this->insertOrUpdateUsermeta($id,'secondary_manager_country_code', $request->secondary_manager_country_code);
        }
        ...
        $this->insertOrUpdateUsermeta($id,'partner_short_name', $request->partner_short_name);

      return redirect()->route('partner.profile')->with('success','Profile has been updated');
     }
  ...
  public function ProfilePhotoRemove(Request $request){
    if(!$request->userId){
        return redirect()->route('partner.settings.editprofile');
    }else{
